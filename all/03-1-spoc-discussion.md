...
- 内碎片：已经分配的内存块内部的未使用部分
- 外碎片：已经分配的内存块之间的未使用部分

2. 最先匹配会越用越慢吗？请说明理由（可来源于猜想或具体的实验）？

- 会，可以设想最初的分配让低地址空间只剩下了较小的外碎片，之后申请较大的内存的时候都不得不遍历整个链表
...
- 采用对换的原因是，处理简单，且不要求程序可以重定位
2. 一个处于等待状态的进程被对换到外存（对换等待状态）后，等待事件出现了。操作系统需要如何响应？
- 将进程从硬盘中读取到内存中，在这个过程中，操作系统将该进程标为等待状态并且调度其他进程。

### 5.5 伙伴系统
1. 伙伴系统的空闲块如何组织？
- 维护大小为$\log_2(n)$的数组，数组的元素是对应大小的空闲区块组成的链表
