...
将程序中不会同时执行的模块放到内存中的同一位置，需要的时候才从外存中载入。

程序员需要控制这种覆盖过程。

2. 什么是交换技术？覆盖与交换有什么不同？

内存不足时由os挑选一个程序，将整个程序换出到硬盘。

3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？

也许可以用于一些硬件能力仍然很受限制的平台。
...
缺页中断用来处理一个页表中不对应物理内存，用来为它分配物理内存和检测可能的非法访问。
...
后者的合法页面不一定对应页帧，当这些页面被访问的时候，os需要为其分配页帧；当物理空间不足的时候，os会选择一些页面，将它们的内容换出到硬盘并回收页帧占用的空间。

### 8.6 缺页异常

1. 缺页异常的处理流程？

缺页中断 -> isr -> 缺页异常处理函数 -> 检查合法性 -> 分配物理页，如物理空间不够则换出某个页。

2. 虚拟页式存储管理中有效存储访问时间是如何计算的？

ETA = 内存访问时间 * (1 - p) + 缺页异常处理时间 * p = 内存访问时间 * (1 - p) + 磁盘访问时间 * (1 + q) * p
...
目前我在qemu上得到的测试结果是，缺页异常可以发生嵌套，而且层数可以远远多于3层。有时候qemu还会表现出一些诡异的结果。最终的结果我还仍不清楚。
...
读取指令：指令可能跨越两个page；读取数据：数据可能跨越两个page；

目录表项invalid会在缺页异常的isr内部处理；TLB缺失在80386上对软件是透明的。这二者都不用考虑。
